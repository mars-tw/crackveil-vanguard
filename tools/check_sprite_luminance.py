...
The command exits non-zero when any character leaves the approved band.
"""
...
def measure(atlas_path: Path) -> tuple[list[dict[str, object]], list[str]]:
    ...
    for index, character_id in enumerate(CHARACTER_IDS):
        ...
        passed = AVERAGE_MIN <= average <= AVERAGE_MAX and near_black < NEAR_BLACK_MAX
        if not passed:
            failures.append(
...
def main() -> None:
    ...
    print("SPRITE_LUMINANCE_GATE avg=0.30-0.80 near_black_luma<0.10 near_black_ratio<35%")
    for record in records:
        print(
            "SPRITE_LUMINANCE character={character} visible={visible_pixels} avg={average_luminance:.4f} "
            "near_black={near_black_ratio:.2%} pass={pass}".format(**record)
        )
    ...
    if failures:
        for failure in failures:
            print("SPRITE_LUMINANCE_FAIL " + failure)
        raise SystemExit(1)
    print(f"SPRITE_LUMINANCE_PASS characters={len(records)}")
