...
func _test_boss_volume_and_projectile() -> void:
	...
	boss.call("_apply_boss_phase_visuals")
	...
	_assert(float(boss.get("hit_flash_timer")) >= 0.079, "hit flash timing missing")
	boss.call("_play_animation_state", &"hurt", true)
	_assert(StringName(boss.get("current_animation_name")) == &"hurt", "real hurt pose animation missing")
	MOBILE_TUNING.set_force_mobile_lod_for_tests(true)
