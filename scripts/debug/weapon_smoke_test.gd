...
const EXPECTED_RECRUIT_WEAPON := "pulse_artificer:pulse_bloom"
...
func _process(_delta: float) -> void:
	...
	if initialized and not recruited and frame_count >= 180:
		_run_recruit_check()

	if initialized and not follow_checked and frame_count >= 330:
		_run_follow_check()
...
func _run_recruit_check() -> void:
	recruited = true
	if not squad_manager.recruit_hero(RECRUIT_HERO_ID):
		_fail("recruit upgrade failed for " + RECRUIT_HERO_ID)
		return

	if squad_manager.get_member_count() != 4:
		_fail("expected 4 heroes after recruit, got %d" % squad_manager.get_member_count())
		return

	print("WEAPON_SMOKE_RECRUIT_COUNT=4")
	_spawn_enemy_clusters_for_members()
...
func _run_follow_check() -> void:
	...
	for slot_index in range(1, min(3, members.size())):
...
func _finish_test() -> void:
	...
	if int(counts.get(EXPECTED_RECRUIT_WEAPON, 0)) <= 0:
		_fail(EXPECTED_RECRUIT_WEAPON + " did not trigger after recruit")
		return
	...
	print("WEAPON_SMOKE_PASS: starting squad, following, weapons, and recruit upgrade verified")
	get_tree().quit(0)
