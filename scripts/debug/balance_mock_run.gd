...
func _run_mock() -> void:
	...
	for second in range(RUN_SECONDS + 1):
		if level_index < LEVEL_TIMES.size() and second == LEVEL_TIMES[level_index]:
			var upgrade := _forced_recruit_upgrade(level_index, squad_members)
			if upgrade.is_empty():
				upgrade = _choose_upgrade()
			_record_upgrade(upgrade)
			match str(upgrade.get("kind", "")):
				"recruit":
					squad_members = min(9, squad_members + 1)
					if squad_members >= 9 and full_squad_time < 0:
						full_squad_time = second
					dps += 21.0
					squad_dps += 21.0
				"qualitative":
		...
		var hp_scale := 1.0
		if second >= 60:
			hp_scale = 1.0 + 0.074 * (float(second - 60) / 60.0)
		...
		var incoming := (density * hp_scale * 1.02) - (dps / 330.0) - ((pickup_quality - 1.0) * 0.18)
		if ELITE_SPAWN_TIMES.has(second):
			incoming += 4.2
	...
	print("BALANCE_MOCK_RESULT seed=%d survival_time=%s elapsed=%d min_hp=%.3f min_hp_time=%d min_hp_before_90=%.3f leader_dps_share=%.3f total_dps=%.1f full_squad_time=%d" % [
		MOCK_SEED,
...
func _forced_recruit_upgrade(level_index: int, squad_members: int) -> Dictionary:
	if squad_members >= 9:
		return {}
	if not [1, 3, 5, 7, 9, 11].has(level_index):
		return {}
	return {"id": "recruit_hero", "kind": "recruit", "weight": 1, "max": 6}


func _available_upgrade_pool() -> Array:
	var numeric_weight := 1.15
	var base_pool: Array = [
		{"id": "recruit_hero", "kind": "recruit", "weight": 8.0, "max": 6},
		...
		{"id": "grenade_cluster", "kind": "qualitative", "weight": 3, "max": 2, "dps": 10.0},
		{"id": "void_anchor", "kind": "qualitative", "weight": 3, "max": 1, "dps": 7.0, "pickup": 0.08},
		{"id": "rail_focus", "kind": "qualitative", "weight": 3, "max": 2, "dps": 12.0},
		{"id": "echo_crescendo", "kind": "qualitative", "weight": 3, "max": 2, "dps": 6.0, "pickup": 0.1},
	...
	if int(upgrade_counts.get("grenade_cluster", 0)) >= 2 and int(upgrade_counts.get("weapon_damage", 0)) >= 3 and int(upgrade_counts.get("evo_cinder_barrage", 0)) <= 0:
		pool.append({"id": "evo_cinder_barrage", "kind": "evolution", "weight": 8, "max": 1, "dps": 16.0})
	if int(upgrade_counts.get("void_anchor", 0)) >= 1 and int(upgrade_counts.get("weapon_damage", 0)) >= 3 and int(upgrade_counts.get("evo_event_horizon", 0)) <= 0:
		pool.append({"id": "evo_event_horizon", "kind": "evolution", "weight": 8, "max": 1, "dps": 12.0})
	if int(upgrade_counts.get("rail_focus", 0)) >= 2 and int(upgrade_counts.get("weapon_damage", 0)) >= 3 and int(upgrade_counts.get("evo_star_piercer", 0)) <= 0:
		pool.append({"id": "evo_star_piercer", "kind": "evolution", "weight": 8, "max": 1, "dps": 17.0})
	if int(upgrade_counts.get("echo_crescendo", 0)) >= 2 and int(upgrade_counts.get("weapon_damage", 0)) >= 3 and int(upgrade_counts.get("evo_resonant_chorus", 0)) <= 0:
		pool.append({"id": "evo_resonant_chorus", "kind": "evolution", "weight": 8, "max": 1, "dps": 11.0, "pickup": 0.12})
	return pool
